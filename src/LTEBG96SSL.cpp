#include "LTEBG96SSL.h"

LTEBG96SSL::LTEBG96SSL()
{
...
LTEBG96SSL::LTEBG96SSL(Stream &atserial, Stream &dserial) : LTEBG96FILE(atserial, dserial)
{

}

bool LTEBG96SSL::SetSSLParameters(unsigned int ssl_index, SSL_Version_t s_version, SSL_Cipher_Suites_t s_ciper, unsigned int negotiation_time)
{
    char cmd[64],buf[32];
...
bool LTEBG96SSL::InitSSL(unsigned int ssl_index, char *ca_cert, char *client_cert, char *client_key, char *err_code)
{
...
bool LTEBG96SSL::OpenSSLSocket(unsigned int pdp_index, unsigned int ssl_index, unsigned int socket_index, char *ip, unsigned int port, Access_Mode_t mode)
{
    char cmd[64],buf[64];
...
bool LTEBG96SSL::SSLSocketSendData(unsigned int socket_index, char *ssl_send_data)
{
    char cmd[16], buf[8];
...
bool LTEBG96SSL::SSLSocketRecvData(unsigned int socket_index, unsigned int ssl_recv_len, char *ssl_recv_data)
{
    char cmd[16],buf[16];
...
bool LTEBG96SSL::CloseSSLSocket(unsigned int socket_index)
{
    char cmd[16],buf[16];
...
bool LTEBG96SSL::QuerySSLSocketStatus(unsigned int socket_index, char *ssl_status)
{
    char cmd[16],buf[16];
...
SSL_Socket_Event_t LTEBG96SSL::WaitCheckSSLSocketEvent(char *event, unsigned int timeout)
{
