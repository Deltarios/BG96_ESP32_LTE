...
LTEBG96TCPIP::LTEBG96TCPIP(Stream &atserial, Stream &dserial) : LTEBG96Common(atserial,dserial)
{

}

bool LTEBG96TCPIP::SetDevAPNParameters(unsigned int pdp_index, Protocol_Type_t type, char *apn, char *usr, char *pwd, Authentication_Methods_t met)
{
    char cmd[64], buf[64];
...
Cmd_Response_t LTEBG96TCPIP::ActivateDevAPN(unsigned int pdp_index)
{
    char cmd[16],buf[8];
...
bool LTEBG96TCPIP::DeactivateDevAPN(unsigned int pdp_index)
{
    char cmd[16],buf[8];
...
bool LTEBG96TCPIP::GetDevAPNIPAddress(unsigned int pdp_index, char *ip)
{
    char cmd[16],buf[8];
...
bool LTEBG96TCPIP::InitAPN(unsigned int pdp_index, char *apn, char *usr, char *pwd, char *err_code)
{
...
bool LTEBG96TCPIP::OpenSocketService(unsigned int pdp_index, unsigned int socket_index, Socket_Type_t socket, char *ip, unsigned int port, unsigned int local_port, Access_Mode_t mode)
{
...
bool LTEBG96TCPIP::CloseSocketService(unsigned int socket_index)
{
    char cmd[16],buf[8];
...
bool LTEBG96TCPIP::SocketSendData(unsigned int socket_index, Socket_Type_t socket, char *data_buf, char *ip, unsigned int port)
{
    char cmd[64],buf[64];
...
bool LTEBG96TCPIP::SocketRecvData(unsigned int socket_index, unsigned int recv_len, Socket_Type_t socket, char *recv_buf)
{
    char cmd[16],buf[16];
...
bool LTEBG96TCPIP::SocketSendHEXData(unsigned int socket_index, char *hex_buf)
{
...
bool LTEBG96TCPIP::SwitchAccessModes(unsigned int socket_index, Access_Mode_t mode)
{
    char cmd[16],buf[16];
...
bool LTEBG96TCPIP::DevPingFunction(unsigned int socket_index, char *host)
{
    char cmd[128],buf[64];
...
bool LTEBG96TCPIP::DevNTPFunction(unsigned int socket_index, char *ntp_ip, unsigned int port, char *time)
{
    char cmd[64],buf[64];
...
bool LTEBG96TCPIP::ConfigDNSServer(unsigned int socket_index, char *primary_addr, char *secondary_addr)
{
    char cmd[128],buf[128];
...
bool LTEBG96TCPIP::DevDNSFunction(unsigned int socket_index, char *domain_name, char *ip)
{
    char cmd[128],buf[128];
...
bool LTEBG96TCPIP::QueryLastErrorCode(char *err_code)
{
    char cmd[16];
...
Socket_Event_t LTEBG96TCPIP::WaitCheckSocketEvent(char *event, unsigned int timeout)
{
