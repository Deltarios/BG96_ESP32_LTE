#include "LTEBG96Common.h"

LTEBG96Common::LTEBG96Common()
{
...
LTEBG96Common::LTEBG96Common(Stream &atserial, Stream &dserial) : LTEBG96Serial(atserial,dserial)
{
...
bool LTEBG96Common::SetDevCommandEcho(bool echo)
{
    const char *cmd;
    if (echo == true){
        cmd = "E1";
...
bool LTEBG96Common::GetDevInformation(char *inf)
{
...
bool LTEBG96Common::GetDevVersion(char *ver)
{
...
bool LTEBG96Common::GetDevIMEI(char *imei)
{
...
Cmd_Response_t LTEBG96Common::SetDevFunctionality(Functionality_t mode)
{
    char cmd[16];
...
bool LTEBG96Common::DevLocalRate(unsigned long &rate, Cmd_Status_t status)
{
    char cmd[16];
...
bool LTEBG96Common::GetDevSimIMSI(char *imsi)
{
...
bool LTEBG96Common::DevSimPIN(char *pin, Cmd_Status_t status)
{
    char cmd[16];
...
bool LTEBG96Common::GetDevSimICCID(char *iccid)
{
...
Net_Status_t LTEBG96Common::DevNetRegistrationStatus()
{
    char cmd[16];
...
bool LTEBG96Common::GetDevNetSignalQuality(unsigned int &rssi)
{
...
Cmd_Response_t LTEBG96Common::ScanOperatorNetwork(char *net)
{
    char cmd[16];
...
Cmd_Response_t LTEBG96Common::DevOperatorNetwork(unsigned int &mode, unsigned int &format, char *oper, Net_Type_t &act, Cmd_Status_t status)
{
    char cmd[16];
...
bool LTEBG96Common::GetDevNetworkInformation(char *type, char *oper, char *band, char *channel)
{
...
bool LTEBG96Common::DevNetPacketCounter(unsigned long &send_bytes, unsigned long &recv_bytes, bool clean)
{
    char cmd[16];
...
bool LTEBG96Common::DevPowerDown()
{
    char cmd[16];
...
bool LTEBG96Common::DevClock(char *d_clock, Cmd_Status_t status)
{
    char cmd[32];
