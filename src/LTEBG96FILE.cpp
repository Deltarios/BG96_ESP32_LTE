#include "LTEBG96FILE.h"

LTEBG96FILE::LTEBG96FILE()
{
...
LTEBG96FILE::LTEBG96FILE(Stream &atserial, Stream &dserial) : LTEBG96TCPIP(atserial, dserial)
{

}

bool LTEBG96FILE::GetFliesSpace(unsigned long &free_bytes, unsigned long &total_bytes)
{
    char cmd[16]; 
...
bool LTEBG96FILE::GetFilesList(char *list)
{
    char cmd[16];
...
bool LTEBG96FILE::DeleteFiles(char *filename)
{
    char cmd[32],buf[32];
...
bool LTEBG96FILE::UploadFiles(char *filename, char *u_file)
{
    char cmd[32],buf[32];
...
bool LTEBG96FILE::DownloadFiles(char *filename, char *d_file)
{
    char cmd[32],buf[32];
...
bool LTEBG96FILE::OpenFile(char *filename, Open_File_Mode_t mode, unsigned int &file_index)
{
    char cmd[32],buf[32];
...
bool LTEBG96FILE::ReadFile(unsigned int file_index, char *read_data)
{
    char cmd[16],buf[8];
...
bool LTEBG96FILE::WriteFile(unsigned int file_index, char *write_data)
{
    char cmd[32],buf[16];
...
bool LTEBG96FILE::CloseFlie(unsigned int file_index)
{
    char cmd[16],buf[8];
...
bool LTEBG96FILE::SetFilePointer(unsigned int file_index, unsigned int offset, Pointer_Mode_t p_mode)
{
    char cmd[32],buf[16];
...
bool LTEBG96FILE::GetFilePointer(unsigned int file_index, unsigned int &offset)
{
    char cmd[32],buf[16];
...
bool LTEBG96FILE::TruncateFile(unsigned int file_index)
{
    char cmd[16],buf[8];
