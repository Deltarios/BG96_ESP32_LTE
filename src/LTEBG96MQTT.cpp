...
LTEBG96MQTT::LTEBG96MQTT(Stream &atserial, Stream &dserial) : LTEBG96SSL(atserial, dserial)
{

}

bool LTEBG96MQTT::SetMQTTConfigureParameters(unsigned int mqtt_index, unsigned int pdp_index, Mqtt_Version_t version, unsigned int keep_alive_time, Mqtt_Session_Type_t type)
{
    char cmd[32], buf[32];
...
bool LTEBG96MQTT::SetMQTTEnableSSL(unsigned int mqtt_index, unsigned int ssl_index, bool enable)
{
    char cmd[32], buf[16];
    strcpy(cmd, MQTT_CONFIG_PARAMETER);
    if(enable){
...
bool LTEBG96MQTT::SetMQTTMessageTimeout(unsigned int mqtt_index, unsigned int pkt_timeout, unsigned int retry_num, bool timeout_notice)
{
    char cmd[32], buf[32];
    strcpy(cmd, MQTT_CONFIG_PARAMETER);
    if(timeout_notice){
...
bool LTEBG96MQTT::SetMQTTAlibabaCloudDeviceInformation(unsigned int mqtt_index, char *product_key, char *device_name, char *device_secret)
{
...
Mqtt_Network_Result_t LTEBG96MQTT::OpenMQTTNetwork(unsigned int mqtt_index, char *host_name, unsigned int port)
{
    char cmd[80], buf[64];
...
Mqtt_Client_Result_Status_t LTEBG96MQTT::CreateMQTTClient(unsigned int mqtt_index, char *client_id, char *username, char *password)
{
    char cmd[128], buf[128];
...
Mqtt_Client_Result_Status_t LTEBG96MQTT::MQTTSubscribeTopic(unsigned int mqtt_index, unsigned int msg_id, char *topic, Mqtt_Qos_t qos)
{
    char cmd[64], buf[64];
