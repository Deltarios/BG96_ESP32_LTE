...
LTEBG96Serial::LTEBG96Serial(Stream &atserial, Stream &dserial) : _atserial(atserial), _dserial(dserial)
{
...
template <class T>
bool LTEBG96Serial::AutoATSerialBand(T &_atserial)
{
...
bool LTEBG96Serial::sendDataAndCheck(const char *data_buf, const char *ok_str, const char *err_str, unsigned int timeout)
{
    delay(100);
...
bool LTEBG96Serial::sendATcommand(const char *command)
{
    delay(100);
    while(_atserial.read()>=0);
    _atserial.write("AT");
...
unsigned int LTEBG96Serial::readResponseByteToBuffer()
{
...
unsigned int LTEBG96Serial::readResponseToBuffer(unsigned int timeout)
{
...
Cmd_Response_t LTEBG96Serial::readResponseAndSearchChr(const char test_chr, unsigned int timeout)
{
    unsigned long start_time = millis();
    unsigned int recv_len = 0;
    cleanBuffer();
...
Cmd_Response_t LTEBG96Serial::readResponseAndSearch(const char *test_str, unsigned int timeout)
{
    unsigned long start_time = millis();
    unsigned int recv_len = 0;
    cleanBuffer();
...
Cmd_Response_t LTEBG96Serial::readResponseAndSearch(const char *test_str, const char *e_test_str, unsigned int timeout)
{
    unsigned long start_time = millis();
    unsigned int recv_len = 0;
    errorCode = -1;
...
Cmd_Response_t LTEBG96Serial::sendAndSearchChr(const char *command, const char test_chr, unsigned int timeout)
{
    for (int i = 0; i < 3; i++){
...
Cmd_Response_t LTEBG96Serial::sendAndSearch(const char *command, const char *test_str, unsigned int timeout)
{
    for (int i = 0; i < 3; i++){
        if(sendATcommand(command)){
...
Cmd_Response_t LTEBG96Serial::sendAndSearch(const char *command, const char *test_str, const char *e_test_str, unsigned int timeout)
{
...
char *LTEBG96Serial::searchStrBuffer(const char *test_str)
{
...
char *LTEBG96Serial::searchChrBuffer(const char test_chr)
{
    int buf_len = strlen((const char *)rxBuffer);
    if (buf_len < RX_BUFFER_LENGTH){
...
bool LTEBG96Serial::returnErrorCode(int &s_err_code)
{
    s_err_code = -1;
    if(errorCode != -1){
...
void LTEBG96Serial::cleanBuffer()
{
    memset(rxBuffer,'\0',RX_BUFFER_LENGTH);
    bufferHead = 0;
}

int LTEBG96Serial::serialAvailable()
{
    unsigned int ret;
