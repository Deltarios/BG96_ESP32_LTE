...
void setup()
{
    ...
    char ssl_error[128];
    while (!BG96LTE.InitSSL(comm_ssl_index, pem_CA, pem_cert, pem_pkey, ssl_error))
    {
    ...
    while (!BG96LTE.SetMQTTEnableSSL(comm_mqtt_index, comm_ssl_index, true))
    {
        DSerial.println("\r\nEnable the SSL Fail!");
        int e_code;
        if (BG96LTE.returnErrorCode(e_code))
    ...
    while (BG96LTE.CreateMQTTClient(comm_mqtt_index, mqtt_client_id, "", "") != 0)
    {
    ...
    while (BG96LTE.MQTTSubscribeTopic(comm_mqtt_index, 0, "/test/bg96", mqtt_qos) != 0)
    {
